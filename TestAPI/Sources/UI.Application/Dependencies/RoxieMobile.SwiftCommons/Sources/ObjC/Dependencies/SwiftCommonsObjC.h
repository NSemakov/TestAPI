// ----------------------------------------------------------------------------
//
//  SwiftCommonsObjC.h
//
//  @author     Alexander Bragin <bragin-av@roxiemobile.com>
//  @copyright  Copyright (c) 2017, Roxie Mobile Ltd. All rights reserved.
//  @link       http://www.roxiemobile.com/
//
// ----------------------------------------------------------------------------

@import Foundation;

// ----------------------------------------------------------------------------

//! Project version number for SwiftCommonsObjC.
FOUNDATION_EXPORT double SwiftCommonsObjCVersionNumber;

//! Project version string for SwiftCommonsObjC.
FOUNDATION_EXPORT const unsigned char SwiftCommonsObjCVersionString[];

// ----------------------------------------------------------------------------
// In this header, you should import all the public headers of your framework
// using statements like #import <FrameworkName/PublicHeader.h>
// ----------------------------------------------------------------------------

//#import <SwiftCommonsObjC/SSZipArchive.h>
//#import <SwiftCommonsObjC/SSZipArchiveDelegate.h>
//#import <SwiftCommonsObjC/unzip.h>
//#import <SwiftCommonsObjC/ioapi.h>
//
//#import <SwiftCommonsObjC/StreamTypedCoder.h>
//
//#import <SwiftCommonsObjC/TryCatchFinally.h>

// ----------------------------------------------------------------------------
